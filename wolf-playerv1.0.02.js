...
`;
  ...
  // Keyboard handler (incluye soporte Smart TV / control remoto)
  handleKeyboard(e) {
    if (['INPUT','TEXTAREA'].includes(e.target.tagName)) return;

    // ── Teclas de control remoto Smart TV ──────────────────
    // Códigos comunes: Samsung Tizen, LG webOS, Android TV, Fire TV
    const tvKeyMap = {
      // Play/Pause — teclas de media
      'MediaPlayPause': 'playpause',
      'MediaPlay': 'play',
      'MediaPause': 'pause',
      'MediaStop': 'stop',
      // Saltar — teclas de media
      'MediaTrackNext': 'next',
      'MediaTrackPrevious': 'prev',
      'MediaFastForward': 'fwd',
      'MediaRewind': 'rwd',
      // OK / Enter en distintos TVs
      'Enter': 'ok',
      'NumpadEnter': 'ok',
      // Atrás / Back (Android TV = Escape, Samsung = GoBack)
      'Escape': 'back',
      'GoBack': 'back',
      'BrowserBack': 'back',
      // Mute
    ...
    const tvAction = tvKeyMap[e.key] || tvKeyMap[e.code];

    // Si el reproductor está en foco activo, manejar acciones de TV
    if (tvAction) {
      switch (tvAction) {
        case 'playpause':
        case 'play':
        ...
        case 'next':
          e.preventDefault();
          // Delegar al sistema de episodios si está disponible
          if (typeof window._wolfTV_nextEpisode === 'function') window._wolfTV_nextEpisode();
          return;
        case 'prev':
          e.preventDefault();
          if (typeof window._wolfTV_prevEpisode === 'function') window._wolfTV_prevEpisode();
          return;
        case 'back':
          e.preventDefault();
          // Si hay fullscreen, salir primero
          if (document.fullscreenElement || document.webkitFullscreenElement) {
            (document.exitFullscreen || document.webkitExitFullscreen).call(document);
          } else if (typeof window._wolfTV_back === 'function') {
            window._wolfTV_back();
          }
          return;
        case 'mute':
        ...
        case 'voldown':
          ...
          return;
        case 'ok':
          // OK sobre el propio player → toggle play/pause
          if (document.activeElement && document.activeElement.closest('#playerContainer')) {
            e.preventDefault();
            if (!document.activeElement.closest('button, select')) {
              this.togglePlay();
              this.showHint(this.video.paused ? '⏸' : '▶');
              this.showUI();
            }
          ...
          return;
        case 'red':
          // Rojo: silenciar/activar audio
          e.preventDefault();
          this.toggleMute();
          this.showHint(this.video.muted ? '🔇' : '🔊');
          return;
        case 'green':
    ...
    // ── Teclas D-Pad cuando el player tiene foco ────────────
    // En el reproductor: flechas controlan seek/volumen según contexto
    const playerActive = this.playerContainer.contains(document.activeElement) ||
