...
`;
  ...
  setupEvents() {
    ...
    this.video.addEventListener('waiting', () => {
      ...
      // Timer de seguridad: ocultar spinner automáticamente después de 10 segundos
    ...
    this.video.addEventListener('seeking', () => {
      // Mostrar spinner al adelantar/retroceder
      this.spinner.classList.add('active');
      // Timer de seguridad
      clearTimeout(this.spinnerTimeout);
      this.spinnerTimeout = setTimeout(() => {
    ...
    this.video.addEventListener('seeked', () => {
      // Ocultar spinner cuando termine de buscar
      setTimeout(() => {
  ...
  takeScreenshot() {
    if (!this.video.videoWidth) { this.showHint('Sin video'); return; }
  ...
  loadHLS(url) {
    if (typeof Hls !== 'undefined' && Hls.isSupported()) {
      this.hlsInstance = new Hls({ 
        enableWorker: true, 
        lowLatencyMode: true,
        // Configuración para mejor manejo de buffering
        maxBufferLength: 30,
      ...
      this.hlsInstance.on(Hls.Events.LEVEL_SWITCHED, (_, d) => {
        ...
        // Ocultar spinner después de cambio de calidad
        setTimeout(() => {
          this.spinner.classList.remove('active');
        }, 500);
      });
      ...
      // Eventos de buffering para manejar spinner
      // NOTA: NO activar spinner en BUFFER_APPENDING ya que ocurre continuamente
      // durante la reproducción normal de HLS y causaría parpadeo/pantalla negra.
      // El spinner solo se activa por los eventos nativos 'waiting' y 'seeking'.
      
      this.hlsInstance.on(Hls.Events.BUFFER_APPENDED, () => {
        // Datos cargados, ocultar spinner si el video puede reproducir
        if (this.video.readyState >= 3) {
          this.spinner.classList.remove('active');
        }
      });
