        ...
        <div class="brightness-dial-wrap">
          <svg class="brightness-dial-svg" id="brightnessDial" viewBox="0 0 200 200" touch-action="none">
            <circle cx="100" cy="100" r="78" fill="none" stroke="rgba(255,255,255,0.08)" stroke-width="14" stroke-linecap="round"
              stroke-dasharray="367.6 490.1" stroke-dashoffset="-183.8" />
            <circle cx="100" cy="100" r="78" fill="none" stroke="var(--accent)" stroke-width="14" stroke-linecap="round"
              id="dialArc"
              stroke-dasharray="0 490.1" stroke-dashoffset="-183.8"
              style="filter: drop-shadow(0 0 8px var(--accent-glow)); transition: stroke 0.2s;" />
            <g id="dialTicks"></g>
            <text x="100" y="97" text-anchor="middle" dominant-baseline="middle" fill="var(--accent)" font-size="34" font-weight="900" font-family="inherit" id="dialText">100</text>
...
`;
  ...
  setupEvents() {
    ...
    this.video.addEventListener('seeking', () => {
      // Mostrar spinner INMEDIATAMENTE al adelantar (sin debounce)
      clearTimeout(this.spinnerDebounce);
      clearTimeout(this.spinnerTimeout);
      this.spinner.classList.add('active');
      
      // Timer de seguridad: ocultar si tarda demasiado
      this.spinnerTimeout = setTimeout(() => {
        this.spinner.classList.remove('active');
      }, 8000);
    });
    ...
    this.video.addEventListener('seeked', () => {
      clearTimeout(this.spinnerDebounce);
      clearTimeout(this.spinnerTimeout);
      // Ocultar spinner cuando el video tiene datos suficientes
      const tryHide = () => {
    ...
    // Window resize listener for dynamic modal height
    window.addEventListener('resize', () => {
      if (this.brightnessModal && this.brightnessModal.classList.contains('open')) {
        const panel = this.brightnessModal.querySelector('.modal-panel');
        if (panel) {
          const vh = window.innerHeight;
          const vw = window.innerWidth;
          const isLandscape = vw > vh;
          const isFullscreen = !!(document.fullscreenElement || document.webkitFullscreenElement);
          if (isFullscreen || (isLandscape && vh <= 600)) {
  ...
  // Progress methods
  updateProgress() {
    if (!this.metadataLoaded || !this.video.duration || isNaN(this.video.duration)) return;
  ...
  // Brightness methods
  openBrightnessModal() {
    // Disparar evento para que el host externo (ej: Series Anime) maneje el modal
    const externalHandled = this.container.dispatchEvent(new CustomEvent('wolf:brightness:open', {
      bubbles: true,
      cancelable: true,
      detail: { value: this.currentBrightness, player: this }
    }));
    // Si el evento fue cancelado (preventDefault), el host lo maneja externamente
    if (!externalHandled) {
    ...
    // JS FIX FOR BUG #1: Dynamic height calculation
    const panel = this.brightnessModal.querySelector('.modal-panel');
    if (panel) {
      const vh = window.innerHeight;
      const vw = window.innerWidth;
  ...
  closeBrightnessModal() {
    ...
    this.container.dispatchEvent(new CustomEvent('wolf:brightness:close', { bubbles: true }));
  }
  ...
  applyBrightness(val) {
    ...
    // Notificar al host del nuevo valor
    this.container.dispatchEvent(new CustomEvent('wolf:brightness:change', {
      bubbles: true,
      detail: { value: val, pct }
    }));
  }

  // API pública para que el host cambie el brillo directamente
  setBrightness(val) {
    this.applyBrightness(Math.max(0.1, Math.min(2, val)));
  }

  // Recalcula el tamaño del SVG según el tamaño real del contenedor
  _resizeDial() {
    if (!this.brightnessDial) return;
    const wrap = this.brightnessDial.closest('.brightness-dial-wrap');
    if (!wrap) return;
    const size = wrap.getBoundingClientRect().width || 130;
    // Escalar fuentes dentro del SVG según el tamaño real
    const scale = size / 200; // viewBox es 200x200
    const textVal = this.brightnessDial.querySelector('#dialText');
    const textLabel = this.brightnessDial.querySelectorAll('text')[1];
    if (textVal) textVal.setAttribute('font-size', Math.round(34 * Math.max(0.6, Math.min(1, scale))));
    if (textLabel) textLabel.setAttribute('font-size', Math.round(11 * Math.max(0.6, Math.min(1, scale))));
  }
  ...
  _initDialTicks() {
    ...
    for (let i = 0; i <= 19; i++) {
      ...
      const isMajor = i % 5 === 0;
      const r1 = isMajor ? 63 : 69;
      const r2 = 76;
      const x1 = 100 + r1 * Math.cos(rad);
      const y1 = 100 + r1 * Math.sin(rad);
      const x2 = 100 + r2 * Math.cos(rad);
      const y2 = 100 + r2 * Math.sin(rad);
      const color = isMajor ? 'rgba(255,255,255,0.35)' : 'rgba(255,255,255,0.12)';
      html += `<line x1="${x1.toFixed(1)}" y1="${y1.toFixed(1)}" x2="${x2.toFixed(1)}" y2="${y2.toFixed(1)}" stroke="${color}" stroke-width="${isMajor ? 2 : 1}"/>`;
    }
    this.dialTicks.innerHTML = html;
  }

  _valToAngle(val) {
    // Arco de 270°: comienza en la posición 7 (135°) y termina en la posición 5 (405°)
  ...
  _updateDial(pct) {
    ...
    // 270° del total del círculo
    const arcLen = frac * (270 / 360) * CIRCUMFERENCE;
    // dashoffset coloca el inicio del arco en la posición 135° (abajo-izquierda)
    // offset = -(135/360)*CIRCUMFERENCE
    const dashOffset = -((135 / 360) * CIRCUMFERENCE);
    this.dialArc.setAttribute('stroke-dasharray', `${arcLen.toFixed(2)} ${CIRCUMFERENCE.toFixed(2)}`);
    this.dialArc.setAttribute('stroke-dashoffset', dashOffset.toFixed(2));
  ...
  onFsChange() {
    ...
    // Modal de brillo: en fullscreen usar position:fixed para cubrir la pantalla completa
    if (this.brightnessModal) {
      this.brightnessModal.style.position = isFs ? 'fixed' : '';
    }
  ...
  showMetadataSpinner() {
    this.spinner.classList.add("active");
    this.spinner.dataset.reason = "metadata";
  }

  hideMetadataSpinner() {
    if (this.spinner.dataset.reason === "metadata") {
      this.spinner.classList.remove("active");
      delete this.spinner.dataset.reason;
    }
  }

  initializeUIControls() {
    this.updateProgress();
    this.container.dispatchEvent(new CustomEvent("wolf:metadata:loaded", {
      detail: {
  ...
  async loadSource(url) {
    ...
    this.video.addEventListener("loadedmetadata", () => {
      ...
      this.hideMetadataSpinner();
      this.initializeUIControls();
      if (this.options.autoplay) {
        this.video.play().catch(e => console.warn("Wolf Player: Autoplay prevented", e));
      }
    }, { once: true });
  ...
  loadHLS(url) {
    if (typeof Hls !== 'undefined' && Hls.isSupported()) {
      // Configuración optimizada: inicio rápido + ABR agresivo
      this.hlsInstance = new Hls({
        enableWorker: true,
        lowLatencyMode: false,

        // ── Buffer: pequeño al inicio para empezar rápido ──
        maxBufferLength: 15,
        ...
        // ── Inicio de calidad: empezar bajo y subir rápido ──
        startLevel: 0,           // Empezar en la calidad MÁS BAJA disponible
        abrEwmaDefaultEstimate: 300000,  // Estimar 300kbps inicial (conservador)
        abrEwmaFastLive: 3,
        abrEwmaSlowLive: 9,
        abrEwmaFastVoD: 3,
        abrEwmaSlowVoD: 9,
        abrBandWidthFactor: 0.85,       // Usar solo el 85% del ancho de banda estimado
        abrBandWidthUpFactor: 0.6,      // Subir calidad con precaución

        // ── Timeouts más cortos para fallo rápido ──
      ...
      // ── Manifiesto parseado: construir menú de calidad ──
      this.hlsInstance.on(Hls.Events.MANIFEST_PARSED, (_, data) => {
        const levels = data.levels || [];
        setTimeout(() => { this.spinner.classList.remove('active'); }, 100);

        // Si solo hay UNA calidad, activar modo anti-stutter
        if (levels.length === 1) {
          console.warn('Wolf Player: Solo un nivel de calidad disponible → modo anti-stutter activo');
          this._startSingleLevelStutterGuard();
        }

        if (this.options.autoplay) {
          setTimeout(() => {
            this.video.play().catch(err => console.warn('Wolf Player: Autoplay failed', err));
          }, 150);
        }
      });

      // ── Manifiesto cargado: menú de calidades ──
      ...
      // ── Buffer appended: ocultar spinner si hay datos ──
      ...
      // ── Errores con recuperación inteligente ──
      this._hlsErrorCount = 0;
      this.hlsInstance.on(Hls.Events.ERROR, (event, data) => {
        if (!data.fatal) {
          this._hlsErrorCount = (this._hlsErrorCount || 0) + 1;
          // Si hay demasiados errores no fatales, intentar recuperar de todos modos
          if (this._hlsErrorCount > 10) {
            this._hlsErrorCount = 0;
            console.warn('Wolf Player: Demasiados errores no fatales, recuperando...');
        ...
        switch (data.type) {
          case Hls.ErrorTypes.NETWORK_ERROR:
            console.error('Wolf Player: HLS network error, recovering...');
            setTimeout(() => this.hlsInstance && this.hlsInstance.startLoad(), 1000);
            break;
          case Hls.ErrorTypes.MEDIA_ERROR:
            console.error('Wolf Player: HLS media error, recovering...');
            this.hlsInstance.recoverMediaError();
            break;
          default:
            console.error('Wolf Player: HLS fatal error', data);
      ...
      console.error('Wolf Player: HLS not supported.');
    }
  }

  // ── Guard anti-stutter para streams de una sola calidad ──
  _startSingleLevelStutterGuard() {
    if (this._stutterGuardActive) return;
    this._stutterGuardActive = true;
    this._lastDropped = 0;
    this._stutterStrikes = 0;

    const check = () => {
      if (!this._stutterGuardActive || !this.hlsInstance) return;
      if (this.video.paused || this.video.ended) { setTimeout(check, 3000); return; }

      // Detectar frames caídos (API VideoPlaybackQuality)
      const quality = this.video.getVideoPlaybackQuality && this.video.getVideoPlaybackQuality();
      if (quality) {
        const dropped = quality.droppedVideoFrames;
        const newDropped = dropped - this._lastDropped;
        this._lastDropped = dropped;

        if (newDropped > 5) {
          this._stutterStrikes++;
          console.warn(`Wolf Player: Stutter detectado (frames caídos: ${newDropped}, strikes: ${this._stutterStrikes})`);

          if (this._stutterStrikes >= 2) {
            // Reducir la carga: pausar carga de buffer y re-arrancar más lento
            console.warn('Wolf Player: Aplicando modo bajo consumo para stream de alta calidad');
            this.hlsInstance.config.maxBufferLength = 8;
            this.hlsInstance.config.maxMaxBufferLength = 12;
            // Forzar re-sync de buffer
            this.hlsInstance.trigger && this.hlsInstance.trigger('hlsMediaDetaching');
            this.hlsInstance.recoverMediaError();
            this._stutterStrikes = 0;
          }
        } else if (newDropped === 0 && this._stutterStrikes > 0) {
          this._stutterStrikes = Math.max(0, this._stutterStrikes - 1);
        }
      }
