  ...
  <div class="player-container" id="playerContainer">
    ...
    <div class="controls" id="controls">
      ...
      <div class="ctrl-row">
        ...
        <!-- SUBTÍTULOS -->
        <button class="opt-btn" id="btnSubtitles">
          <svg viewBox="0 0 24 24">
    ...
    <div class="subtitles-display" id="subtitlesDisplay"></div>
...
`;
  ...
  setupDOM() {
    ...
    this.btnSubtitles = this.container.querySelector('#btnSubtitles');
    this.labelSubtitles = this.container.querySelector('#labelSubtitles');
    this.subtitlesDisplay = this.container.querySelector('#subtitlesDisplay');
    this.subtitlesMenu = this.container.querySelector('#subtitlesMenu');
    this.btnSubtitlesMenuClose = this.container.querySelector('#btnSubtitlesMenuClose');
  }
  ...
  setupEvents() {
    ...
    // Subtitles
    this.btnSubtitles.addEventListener('click', (e) => {
      e.stopPropagation();
      this.toggleSubtitlesMenu();
    });
    this.btnSubtitlesMenuClose.addEventListener('click', (e) => {
      e.stopPropagation();
      this.closeSubtitlesMenu();
    });
    // Agregar listener al item "Desactivados"
    const disabledItem = this.subtitlesMenu.querySelector('[data-track="-1"]');
    if (disabledItem) {
      disabledItem.addEventListener('click', (e) => {
        e.stopPropagation();
        this.selectSubtitleTrack(-1);
      });
    }
    document.addEventListener('click', (e) => {
      if (this.subtitlesMenu && this.subtitlesMenu.classList.contains('open')) {
        if (!this.subtitlesMenu.contains(e.target) && !this.btnSubtitles.contains(e.target)) {
          this.closeSubtitlesMenu();
        }
      }
    });
  ...
  async loadSource(url) {
    ...
    if (/^magnet:|\.torrent$/i.test(url)) this.loadTorrent(url);
      ...
      // Detectar subtítulos cuando el video esté listo
  ...
  loadHLS(url) {
    if (typeof Hls !== 'undefined' && Hls.isSupported()) {
      ...
      this.hlsInstance.on(Hls.Events.MANIFEST_PARSED, () => {
        // Detectar subtítulos cuando el manifiesto esté cargado
        setTimeout(() => this.detectSubtitleTracks(), 500);
  ...
  loadDASH(url) {
    if (typeof dashjs !== 'undefined') {
      ...
      this.dashInstance.on(dashjs.MediaPlayer.events.STREAM_INITIALIZED, () => {
        ...
        // Detectar subtítulos cuando el stream esté inicializado
        setTimeout(() => this.detectSubtitleTracks(), 500);
      });
    } else {
  ...
  detectSubtitleTracks() {
    ...
    const menuBody = this.subtitlesMenu.querySelector('.subtitles-menu-body');
    ...
    Array.from(this.video.textTracks).forEach((track, index) => {
      ...
      track.addEventListener('cuechange', () => {
        if (track === this.currentSubtitleTrack) {
          this.updateSubtitlesDisplay();
        }
      });
    ...
    if (this.subtitlesEnabled) {
      if (forcedSpanishIndex !== -1) {
        ...
        this.selectSubtitleTrack(forcedSpanishIndex);
        ...
        this.selectSubtitleTrack(forcedIndex);
        ...
        this.selectSubtitleTrack(firstSpanishIndex);
  ...
  selectSubtitleTrack(trackIndex) {
    ...
    const items = this.subtitlesMenu.querySelectorAll('.subtitles-menu-item');
    items.forEach(item => {
    ...
    if (trackIndex === -1) {
      ...
      this.subtitlesDisplay.textContent = '';
      this.subtitlesDisplay.classList.remove('active');
      ...
      this.showHint('Subtítulos desactivados');
    } else {
      ...
      if (track) {
        ...
        const selectedItem = this.subtitlesMenu.querySelector(`[data-track="${trackIndex}"]`);
        if (selectedItem) {
        ...
        this.showHint(`Subtítulos: ${label}`);
      }
    }
  ...
  updateSubtitlesDisplay() {
    if (!this.currentSubtitleTrack || !this.subtitlesEnabled || this.currentSubtitleTrack.mode === 'disabled') {
      this.subtitlesDisplay.textContent = '';
      this.subtitlesDisplay.classList.remove('active');
      return;
    }

    const activeCues = this.currentSubtitleTrack.activeCues;
    ...
    if (activeCues && activeCues.length > 0) {
      ...
      this.subtitlesDisplay.innerHTML = text;
      this.subtitlesDisplay.classList.add('active');
    } else {
      this.subtitlesDisplay.textContent = '';
      this.subtitlesDisplay.classList.remove('active');
    }
  ...
  updateSubtitlesLabel() {
    if (!this.labelSubtitles) return;

    if (!this.subtitlesEnabled || !this.currentSubtitleTrack) {
      this.labelSubtitles.textContent = 'Subtítulos';
    } else {
      const trackLabel = this.currentSubtitleTrack.label || this.currentSubtitleTrack.language || 'Subtítulos';
      this.labelSubtitles.textContent = trackLabel;
    }
  }

  toggleSubtitlesMenu() {
    if (this.subtitlesMenu.classList.contains('open')) {
      this.closeSubtitlesMenu();
    } else {
      this.openSubtitlesMenu();
    }
  }

  openSubtitlesMenu() {
    this.subtitlesMenu.classList.add('open');
  }

  closeSubtitlesMenu() {
    this.subtitlesMenu.classList.remove('open');
  }

  addSubtitleTrack(src, label, language = '', isDefault = false) {
